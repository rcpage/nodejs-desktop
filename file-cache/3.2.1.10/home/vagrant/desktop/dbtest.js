...
class test extends Endpoint {
  ...
  GET(){
    if(!this.req.query.q){
      ...
      this.res.end('<form action="//rusty.com:1011/test?q=add.accounts.user" method="POST"><input name="email"/><input name="password" type="password"/><input type="submit"/></form>');
    } else {
      this.executeAction();
    }
  }
